                ...
                <>
                    ...
                    <View style={styles.feesRow}>
                        ...
                        <Text allowFontScaling={false} style={styles.feesAmount}>
                            {item.fees?.amount != null
                                ? `Rs. ${item.fees.amount.toLocaleString()}`
                                : 'Not set'}
                        </Text>
                    </View>
...
const DarseNizami = ({ navigation }) => {
                ...
                classList.map(async cls => {
                    try {
                        const [subRes, feeRes] = await Promise.all([
                            api.get(`/darse-nizami/subject/by-class/${cls._id}`),
                            api.get(`/darse-nizami/fees/${cls._id}`),
                        ]);
                        return {
                            ...cls,
                            subjects: Array.isArray(subRes) ? subRes : (subRes.data || []),
                            fees: Array.isArray(feeRes) ? feeRes[0] : (feeRes.data || feeRes),
                        };
                    } catch {
                        return { ...cls, subjects: [], fees: null };
                    }
                })
            );
    ...
    const handleEnroll = async (classId) => {
        ...
        try {
            const res = await api.post('/darse-nizami/enroll', { classId: classId });
            setModal({
